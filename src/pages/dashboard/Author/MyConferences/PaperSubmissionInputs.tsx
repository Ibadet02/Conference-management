...
const PaperSubmissionInputs: React.FC<PaperSubmissionInputsProps> = ({
  ...
  useEffect(() => {
    if(authUser?.uid){
      setPaperSubmissionData((prev) => ({...prev, correspondingAuthor: authUser?.uid}))
    }
  }, [authUser?.uid])
    ...
    <form onSubmit={handleSubmitPaper}>
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        <TextField
          ...
          style={{ width: "100%" }}
        />
        <div style={{color: '#6e6e6e',flex: 1, display: 'flex', padding: '5px 0px', justifyContent: 'flex-end', transform: 'translateY(-10px)'}}>{paperSubmissionData?.abstract?.length}/1000</div>
        ...
        <Box sx={{ width: "100%" }}>
          <FormControl fullWidth>
            ...
            >
              {users?.filter((e) => e?.id !== authUser?.uid).map((user: any) => (
